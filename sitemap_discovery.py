...
def patch(updates):
    ...
    for i in range(0,len(updates),10):
        r=requests.patch(url,headers={**HEADERS,"Content-Type":"application/json"},
                         json={"records":updates[i:i+10],"typecast":True},timeout=30); r.raise_for_status(); time.sleep(0.25)
def post(records):
    url=f"{API}/{BASE}/{quote(TABLE)}"
    for i in range(0,len(records),10):
        r=requests.post(url,headers={**HEADERS,"Content-Type":"application/json"},
                        json={"records":records[i:i+10],"typecast":True},timeout=30); r.raise_for_status(); time.sleep(0.25)
